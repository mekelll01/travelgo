...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !$pay) {
    ...
    if (!in_array($metode, $allowed)) {
        ...
        $stmtPay->bind_param('isssds', $booking_id, $metode, $bank_dompet,
                              $kode_bayar, $booking['total_harga'], $batas_bayar);

        if ($stmtPay->execute()) {
            // Reload untuk tampilkan instruksi bayar
    ...
    <div class="row g-4">
      ...
      <div class="col-lg-8">
        ...
        <form method="POST" action="" id="payForm">
          ...
          <!-- Kode Promo -->
          <div class="pay-section fade-up" style="margin-top:16px;">
            <div class="pay-section-title">
              <i class="bi bi-tag-fill" style="color:#FBBF24"></i>
              Kode Promo
            </div>
            <div style="display:flex;gap:10px;">
              <input type="text" id="kodePromoInput" placeholder="Masukkan kode promo"
                     style="flex:1;padding:11px 14px;border:1.5px solid var(--gray-200);
              ...
              <button type="button" onclick="validasiPromo()"
                      style="padding:11px 20px;background:var(--blue);color:#fff;
                             border:none;border-radius:var(--radius-md);
            ...
            <div id="promoResult" style="margin-top:10px;display:none;"></div>
            <input type="hidden" id="kodePromoApplied" name="kode_promo" value="">
            <input type="hidden" id="diskonRp" name="diskon_rp" value="0">
          </div>
...
function validasiPromo() {
  const kode   = document.getElementById('kodePromoInput').value.trim();
  const result = document.getElementById('promoResult');
  const total  = <?= $booking['total_harga'] ?? 0 ?>;

  if (!kode) { result.style.display='none'; return; }

  result.style.display = 'block';
  result.innerHTML = '<span style="font-size:.82rem;color:var(--gray-400);">Memvalidasi...</span>';
      ...
      if (data.success) {
        result.innerHTML = `
          <div style="background:rgba(16,185,129,.08);border:1px solid rgba(16,185,129,.2);
                      ...
                      font-size:.83rem;color:#065F46;display:flex;align-items:center;gap:8px;">
            <i class="bi bi-check-circle-fill" style="color:var(--success);"></i>
            <div>
              <strong>${data.judul}</strong><br>
              Hemat <strong>${data.diskon_fmt}</strong> → Total jadi <strong>${data.total_fmt}</strong>
            </div>
          </div>`;
        document.getElementById('kodePromoApplied').value = kode;
        document.getElementById('diskonRp').value = data.diskon_rp;
        // Update total di sidebar
        const totalEl = document.querySelector('.os-row.total span:last-child');
        if (totalEl) totalEl.textContent = data.total_fmt;
      } else {
        result.innerHTML = `
          <div style="background:rgba(239,68,68,.08);border:1px solid rgba(239,68,68,.2);
                      border-radius:var(--radius-md);padding:10px 14px;
                      font-size:.83rem;color:#991B1B;display:flex;align-items:center;gap:8px;">
            <i class="bi bi-x-circle-fill"></i> ${data.message}
          </div>`;
        document.getElementById('kodePromoApplied').value = '';
        document.getElementById('diskonRp').value = 0;
      }
    })
    .catch(() => {
      result.innerHTML = '<span style="font-size:.82rem;color:var(--danger);">Gagal menghubungi server.</span>';
    });
}

// Enter key di input promo
document.getElementById('kodePromoInput')?.addEventListener('keydown', e => {
