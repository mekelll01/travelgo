...
if ($asal_id && $tujuan_id && $tgl_valid) {
    ...
    // Cari hari dalam seminggu dari tanggal yang dipilih (1=Senin ... 7=Minggu)
    $hariTerpilih = (int)date('N', strtotime($tgl_berangkat));

    $kelasWhere = $kelas_filter ? "AND j.kelas = ?" : "";

    // Query mendukung 3 mode:
    // 1. Jadwal tanggal spesifik (tanggal_berangkat = ?)
    // 2. Jadwal berulang setiap hari (hari_operasi = 0)
    // 3. Jadwal berulang hari tertentu (hari_operasi = hari_terpilih)
    $sql = "
        SELECT
            j.id, j.kode_jadwal,
            CASE
                WHEN j.tanggal_berangkat IS NOT NULL AND j.hari_operasi = 0 AND j.tanggal_berangkat != '1970-01-01'
                     AND j.tanggal_berangkat = ? THEN j.tanggal_berangkat
                ELSE ?
    ...
    if ($kelas_filter) {
        // 8 param utama + 1 kelas = 9 total: ss iii s i i s
        $stmt = $conn->prepare($sql);
        $stmt->bind_param('ssiiiisis',
            $tgl_berangkat, $tgl_berangkat,
            $asal_id, $tujuan_id, $jenis_id,
            $tgl_berangkat, $hariTerpilih,
            $minPenumpang, $kelas_filter
        );
    } else {
        // 8 param utama: ss iii s i i
        $stmt = $conn->prepare($sql);
    ...
    // ---- Catat search log ke MongoDB ----
    if (isLogin() && $totalResults >= 0) {
        try {
            logSearch($_SESSION['user_id'], $jenis_id, $asal_id, $tujuan_id, $tgl_berangkat);
      ...
      <?php if ($asal_id && $tujuan_id && $tgl_valid): ?>
        ...
        <?php if (empty($results)): ?>
          <div class="empty-state">
            <div class="icon">
              <?php
                $emptyIcons = [1=>'✈️',2=>'🚂',3=>'🚌',4=>'🚢',5=>'🚐'];
                echo $emptyIcons[$jenis_id] ?? '🎫';
              ?>
            </div>
        ...
        <?php else: ?>
          <?php
            ...
            // Map gambar per jenis transportasi
            $imgMap = [
              1 => APP_URL . '/assets/img/transport/pesawat.jpeg',
              2 => APP_URL . '/assets/img/transport/kereta.jpg',
              3 => APP_URL . '/assets/img/transport/bus.jpg',
              4 => APP_URL . '/assets/img/transport/kapal.jpg',
              5 => APP_URL . '/assets/img/transport/travel.jpg',
            ];
            $imgFallback = [1=>'✈️',2=>'🚂',3=>'🚌',4=>'🚢',5=>'🚐'];

            // Fasilitas default per kelas
            $fasilitasMap = [
              'Ekonomi'   => [['icon'=>'wifi',        'label'=>'WiFi'],   ['icon'=>'cup-hot',  'label'=>'Snack'],  ['icon'=>'lightning-charge','label'=>'Charger']],
              'Bisnis'    => [['icon'=>'wifi',        'label'=>'WiFi'],   ['icon'=>'cup-hot',  'label'=>'Makan'],  ['icon'=>'lightning-charge','label'=>'Charger'], ['icon'=>'tv',       'label'=>'TV']],
              'Eksekutif' => [['icon'=>'wifi',        'label'=>'WiFi'],   ['icon'=>'cup-hot',  'label'=>'Makan'],  ['icon'=>'lightning-charge','label'=>'Charger'], ['icon'=>'tv',       'label'=>'TV'], ['icon'=>'star','label'=>'Premium']],
              'Reguler'   => [['icon'=>'lightning-charge','label'=>'Charger'],['icon'=>'cup-hot','label'=>'Snack']],
            ];

            // Cari jadwal lain kelas berbeda dari operator & rute yang sama
            $kelasLainMap = [];
            foreach ($results as $ri) {
              $key = $ri['operator_nama'] . '|' . $ri['kota_asal_nama'] . '|' . $ri['kota_tujuan_nama'];
              if (!isset($kelasLainMap[$key])) $kelasLainMap[$key] = [];
              $kelasLainMap[$key][$ri['kelas']] = [
                'id'    => $ri['id'],
                'harga' => $ri['harga_dewasa'],
                'kelas' => $ri['kelas'],
              ];
            }
          ?>
          ...
          <?php foreach ($results as $i => $r):
            ...
            $kelasLower = strtolower($r['kelas']);
            $fas        = $fasilitasMap[$r['kelas']] ?? $fasilitasMap['Reguler'];
            $imgSrc     = $imgMap[$jenis_id] ?? '';

            // Kelas lain dari operator yang sama
            $keyOp      = $r['operator_nama'] . '|' . $r['kota_asal_nama'] . '|' . $r['kota_tujuan_nama'];
            $kelasLain  = array_filter($kelasLainMap[$keyOp] ?? [], fn($k) => $k['kelas'] !== $r['kelas']);
          ...
          ?>
            <div class="result-card <?= $isCheapest ? 'cheapest' : '' ?>">
              ...
              <?php if ($isCheapest): ?>
                <div class="cheapest-badge"><i class="bi bi-lightning-charge-fill"></i> Termurah</div>
              <?php endif; ?>

              <!-- Image strip -->
              <div class="rc-image-strip">
                <?php if ($imgSrc): ?>
                  <img src="<?= $imgSrc ?>" alt="<?= clean($jenisNama) ?>"
                       onerror="this.style.display='none';this.nextElementSibling.style.display='flex';">
                  <div class="rc-img-fallback" style="display:none;"><?= $imgFallback[$jenis_id] ?? '🎫' ?></div>
                <?php else: ?>
                  <div class="rc-img-fallback"><?= $imgFallback[$jenis_id] ?? '🎫' ?></div>
                <?php endif; ?>
                <div class="rc-img-overlay"></div>
                <div class="rc-img-info">
                  <div>
                    <div class="rc-op-name"><?= clean($r['operator_nama']) ?></div>
                    <div class="rc-op-kode"><?= clean($r['kode_jadwal']) ?></div>
                  </div>
                  <div class="rc-kelas-pill <?= $kelasLower ?>"><?= clean($r['kelas']) ?></div>
                </div>
              </div>
              ...
              <div class="rc-body">
                ...
                <div style="flex:1;min-width:160px;">
                  <div class="rc-times">
                    <div class="rc-time-block">
                      <div class="rc-time"><?= $jamBrgkt ?></div>
                      <div class="rc-city"><?= clean($r['kota_asal_kode'] ?: substr($r['kota_asal_nama'],0,3)) ?></div>
                    </div>
                    <div class="rc-arrow">
                      <div class="rc-arrow-line"></div>
                      <div class="rc-durasi"><?= formatDurasi($r['durasi_menit']) ?></div>
                      <div class="rc-langsung">Langsung</div>
                    </div>
                    <div class="rc-time-block">
                      <div class="rc-time"><?= $jamTiba ?></div>
                      <div class="rc-city"><?= clean($r['kota_tujuan_kode'] ?: substr($r['kota_tujuan_nama'],0,3)) ?></div>
                    </div>
                  </div>

                  <!-- Fasilitas -->
                  <div class="rc-fasilitas">
                    <?php foreach ($fas as $f): ?>
                      <span class="rc-fas-chip">
                        <i class="bi bi-<?= $f['icon'] ?>"></i> <?= $f['label'] ?>
                      </span>
                    <?php endforeach; ?>
                    <?php if ($kursiSisa <= 10 && $kursiSisa > 0): ?>
                      <span class="rc-fas-chip" style="border-color:rgba(239,68,68,.3);color:var(--danger);background:rgba(239,68,68,.05);">
                        <i class="bi bi-exclamation-circle"></i> Hampir habis!
                      </span>
                    <?php endif; ?>
                  </div>
                </div>
                ...
                <div style="text-align:center;min-width:100px;">
                  <div style="font-size:.78rem;font-weight:700;color:var(--navy);">
                    <?= date('d M Y', strtotime($tgl_berangkat)) ?>
                  </div>
                  <div style="font-size:.7rem;color:var(--gray-400);margin-top:2px;">
                    <?= date('l', strtotime($tgl_berangkat)) ?>
                  </div>
                  <div class="rc-seat <?= $kursiSisa <= 10 ? 'low' : 'ok' ?>" style="justify-content:center;margin-top:8px;">
                ...
                <!-- Harga & tombol -->
                <div class="rc-right">
                  <div class="rc-price-label">Mulai dari</div>
                  <div class="rc-price"><?= formatRupiah($r['harga_dewasa']) ?></div>
                  <div class="rc-price-sub">/orang</div>
                  <?php if ($jml_anak > 0): ?>
                    <div style="font-size:.68rem;color:var(--gray-400);">Anak: <?= formatRupiah($r['harga_anak']) ?></div>
                  <?php endif; ?>
                  <?php if ($jml_dewasa + $jml_anak > 1): ?>
                    <div class="rc-total">Total: <?= formatRupiah($totalHarga) ?></div>
                  <?php endif; ?>
              ...
              <?php if (!empty($kelasLain)): ?>
                <div class="rc-other-kelas">
                  <span class="rc-other-label">Kelas lain:</span>
                  <?php foreach ($kelasLain as $kl):
                    $klUrl = APP_URL . '/pages/booking.php?' . http_build_query([
                      'jadwal_id'     => $kl['id'],
                      'dewasa'        => $jml_dewasa,
                      'anak'          => $jml_anak,
                      'tgl_berangkat' => $tgl_berangkat,
                  ...
                  ?>
                    <a href="<?= $klUrl ?>" class="rc-kelas-option">
                      <i class="bi bi-star" style="font-size:.75rem;color:var(--gray-400);"></i>
                      <?= clean($kl['kelas']) ?>
                      <span class="price"><?= formatRupiah($kl['harga']) ?></span>
                    </a>
                  <?php endforeach; ?>
                </div>
              <?php endif; ?>
